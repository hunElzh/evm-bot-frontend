...
export default items.map((item) => ({
  ...
  estimateGasFee: (signer: Wallet) => estimateGasFee(signer, getCalls(item.contractAddress)),
  sendTransaction: (signer: Wallet) => sendTransaction(signer, getCalls(item.contractAddress)),
})) as Module[]
